...
export async function GET(request: Request) {
  ...
  const { data: pedido, error } = await admin.from("pedidos").select("id, cliente_nome, status, total, created_at, tipo_atendimento, empresas(nome, tempo_entrega_minutos)").eq("codigo_acompanhamento", codigo).maybeSingle();
