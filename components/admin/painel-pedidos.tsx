...
type Pedido = { id: string; cliente_nome: string; cliente_telefone: string | null; tipo_entrega: string; status: string; pagamento: string; total: number; created_at: string; codigo_retirada?: string };
...
export function PainelPedidos() {
  ...
  async function carregar(mostrarCarregando = true) {
    ...
    const [pedidosResposta, produtosResposta] = await Promise.all([
      supabase.from("pedidos").select("id, cliente_nome, cliente_telefone, tipo_entrega, status, pagamento, total, created_at, codigo_retirada").eq("empresa_id", perfil.empresa_id).order("created_at", { ascending: false }),
