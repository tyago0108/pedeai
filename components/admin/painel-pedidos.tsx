...
export function PainelPedidos() {
  ...
  async function confirmarEntrega(pedido: Pedido) {
    const codigo = prompt("Informe o código de entrega/retirada apresentado pelo cliente:")?.trim().toUpperCase();
    if (!codigo) return;
    if (codigo !== pedido.codigo_retirada) return setErro("O código informado não confere com este pedido.");
    await atualizarStatus(pedido.id, "entregue");
  }
  async function redefinirCliente() {
    const telefoneCliente = prompt("WhatsApp do cliente (com DDD):");
    if (!telefoneCliente) return;
    const { data: { session } } = await supabase.auth.getSession();
    const resposta = await fetch("/api/restaurante/clientes", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token ?? ""}` }, body: JSON.stringify({ telefone: telefoneCliente }) });
    const resultado = await resposta.json();
    if (!resposta.ok) return setErro(resultado.error ?? "Não foi possível redefinir o acesso.");
    window.alert(`Novo código de ${resultado.nome}: ${resultado.codigo}\nInforme este código ao cliente somente após confirmar a identidade dele.`);
  }
