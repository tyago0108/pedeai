...
type Pedido = {
  ...
  tipo_atendimento: string; status: string; status_atualizado_em: string; pagamento: string;
  pago: boolean; total: number; created_at: string; endereco_entrega: string | null;
  observacao: string | null; taxa_entrega: number; taxa_pagamento: number; troco_para: number | null;
};
...
type ItemBalcao = { produtoId: string; quantidade: number };
...
const statusAtivos = ["recebido", "preparando", "pronto_para_retirada", "saiu_para_entrega"];
const rotulos: Record<string, string> = { recebido: "Novo", preparando: "Em produção", pronto_para_retirada: "Pronto para retirada", saiu_para_entrega: "Em rota", finalizado: "Finalizado" };
...
function BarraStatus({ status }: { status: string }) {
  const sequencia = status === "pronto_para_retirada"
    ? ["recebido", "preparando", "pronto_para_retirada", "finalizado"]
    : etapas;
  const atual = sequencia.indexOf(status);
  return <div className="mt-3 flex gap-1" aria-label={`Status: ${rotulos[status] ?? status}`}>
    {sequencia.slice(0, 3).map((etapa, indice) => <span key={etapa} className={`h-1.5 flex-1 rounded-full ${atual >= indice ? "bg-orange-500" : "bg-stone-200"}`} />)}
  </div>;
}
...
export function PainelPedidos() {
  ...
  const carregar = useCallback(async () => {
    ...
    if (lista.error) return setErro("Banco desatualizado ou indisponível. Execute as migrações 016 e 017 no Supabase e recarregue a página.");
    const ativos: Pedido[] = (lista.data ?? []).map((pedido) => ({
      ...pedido, numero_pedido: Number(pedido.numero_pedido), total: Number(pedido.total),
      taxa_entrega: Number(pedido.taxa_entrega ?? 0), taxa_pagamento: Number(pedido.taxa_pagamento ?? 0),
  ...
  useEffect(() => {
    if (!alerta) return;
    const timeout = window.setTimeout(() => setAlerta(null), 12000);
  ...
  function adicionarItemBalcao(produtoId: string) {
    setItensBalcao((atuais) => {
      const existe = atuais.find((item) => item.produtoId === produtoId);
      return existe
        ? atuais.map((item) => item.produtoId === produtoId ? { ...item, quantidade: item.quantidade + 1 } : item)
        : [...atuais, { produtoId, quantidade: 1 }];
    });
  }

  function mudarQuantidadeBalcao(produtoId: string, delta: number) {
    setItensBalcao((atuais) => atuais.flatMap((item) => {
  ...
  async function salvarBalcao(event: FormEvent<HTMLFormElement>) {
    ...
    const itens = itensBalcao.map((item) => ({ ...item, produto: produtos.find((produto) => produto.id === item.produtoId) }))
      .filter((item): item is ItemBalcao & { produto: Produto } => Boolean(item.produto));
    if (!empresaId || itens.length === 0) return setErro("Adicione pelo menos um item ao pedido de balcão.");

    const atendimento = String(dados.get("atendimento"));
    const { data: pedido, error } = await supabase.from("pedidos").insert({
      empresa_id: empresaId,
      cliente_nome: String(dados.get("cliente")),
      cliente_telefone: String(dados.get("telefone")) || null,
      tipo_atendimento: atendimento,
      status: "recebido",
      pagamento: String(dados.get("pagamento")),
      observacao: String(dados.get("observacao")) || null,
      total: totalBalcao,
      endereco_entrega: atendimento === "entrega" ? String(dados.get("endereco")) : null,
    }).select("id").single();
    if (error || !pedido) return setErro(error?.message || "Não foi possível criar o pedido de balcão.");

    const { error: erroItens } = await supabase.from("itens_pedido").insert(itens.map((item) => ({
      pedido_id: pedido.id, produto_id: item.produto.id, nome_produto: item.produto.nome,
      quantidade: item.quantidade, preco_unitario: item.produto.preco,
    })));
    if (erroItens) return setErro("Pedido criado, mas não foi possível incluir os itens.");
    setItensBalcao([]);
    fecharBalcao();
    void carregar();
  }
  ...
  async function salvarEdicao(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("pedidos").update({
      cliente_nome: edicao.cliente_nome, cliente_telefone: edicao.cliente_telefone,
      tipo_atendimento: edicao.tipo_atendimento, pagamento: edicao.pagamento,
      endereco_entrega: edicao.tipo_atendimento === "entrega" ? edicao.endereco_entrega : null,
    }).eq("id", edicao.id).eq("status", "recebido");
  ...
  return <main className="min-h-screen bg-stone-100 pb-24 text-stone-900">
    <header className="border-b bg-white p-4">
      <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3">
        <div><p className="text-xs font-bold text-orange-500">PEDEAI · OPERAÇÃO</p><h1 className="text-xl font-bold">Pedidos em andamento</h1></div>
        <div className="flex gap-2">
          <Link href="/restaurante/historico" className="rounded-xl border bg-white px-3 py-2 text-sm font-bold">Histórico</Link>
          <button onClick={() => { setItensBalcao([]); setBuscaBalcao(""); setBalcao(true); }} className="rounded-xl bg-orange-500 px-3 py-2 text-sm font-bold text-white">+ Balcão</button>
          <button onClick={() => { if (pedidosNovos[0]) void verPedido(pedidosNovos[0]); }} className={`rounded-xl border px-3 py-2 text-sm font-bold ${novos.length ? "animate-pulse border-orange-500 bg-orange-50 text-orange-800" : "bg-white"}`}>Novos {novos.length ? `(${novos.length})` : ""}</button>
        </div>
      </div>
    </header>
    ...
    <section className="mx-auto max-w-5xl p-4">
      ...
      {pedidosNovos.length > 0 && <div className="mb-3 flex justify-between rounded-2xl border border-orange-200 bg-orange-50 p-3 text-sm text-orange-900"><span><strong>{pedidosNovos.length} pedido(s) novo(s)</strong> aguardando atendimento.</span><button onClick={() => setNovos([])} className="font-bold underline">Limpar aviso</button></div>}
      {pedidos.length === 0 ? <p className="rounded-2xl bg-white p-6 text-center text-sm text-stone-500 shadow-sm">Nenhum pedido em andamento.</p> : <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
        {pedidos.map((pedido) => <article key={pedido.id} onClick={() => void verPedido(pedido)} className={`cursor-pointer border-b border-stone-100 p-4 transition hover:bg-stone-50 ${novos.includes(pedido.id) ? "animate-pulse bg-orange-50" : ""}`}>
          <div className="flex justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><strong className="text-orange-700">{numero(pedido.numero_pedido)}</strong><strong className="truncate">{pedido.cliente_nome}</strong>{pedido.pago && <span className="rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-bold text-green-800">PAGO</span>}</div><p className="mt-1 text-xs text-stone-500">{new Date(pedido.created_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} · {rotulos[pedido.status]} há {tempoDecorrido(pedido.status_atualizado_em, agora)}</p></div><strong className="whitespace-nowrap text-orange-700">{moeda.format(pedido.total)}</strong></div>
          <BarraStatus status={pedido.status} />
        </article>)}
      </div>}
    </section>

    {alerta && <aside className="fixed right-4 top-4 z-40 w-[calc(100%-2rem)] max-w-sm rounded-2xl border-2 border-orange-400 bg-white p-4 shadow-xl"><button onClick={() => setAlerta(null)} className="float-right text-xl text-stone-500">×</button><p className="text-xs font-bold text-orange-600">NOVO PEDIDO {numero(alerta.numero_pedido)}</p><p className="mt-1 font-bold">{alerta.cliente_nome} · {moeda.format(alerta.total)}</p><button onClick={() => { setAlerta(null); void verPedido(alerta); }} className="mt-3 w-full rounded-xl bg-orange-500 py-3 text-sm font-bold text-white">Ver pedido</button></aside>}

    {detalhe && <div className="fixed inset-0 z-40 grid place-items-center bg-black/40 p-4"><section className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl"><div className="flex justify-between"><div><p className="text-xs font-bold text-orange-600">PEDIDO {numero(detalhe.pedido.numero_pedido)}</p><h2 className="text-xl font-bold">{detalhe.pedido.cliente_nome}</h2></div><button onClick={() => setDetalhe(null)} className="text-xl text-stone-500">×</button></div><p className="mt-2 text-sm text-stone-500">Feito às {new Date(detalhe.pedido.created_at).toLocaleString("pt-BR")} · etapa atual há {tempoDecorrido(detalhe.pedido.status_atualizado_em, agora)}</p><BarraStatus status={detalhe.pedido.status} /><div className="mt-4 rounded-2xl bg-stone-50 p-3 text-sm"><p><b>WhatsApp:</b> {detalhe.pedido.cliente_telefone || "Não informado"}</p><p className="mt-1"><b>Atendimento:</b> {detalhe.pedido.tipo_atendimento}</p>{detalhe.pedido.endereco_entrega && <p className="mt-1"><b>Endereço:</b> {detalhe.pedido.endereco_entrega}</p>}<p className="mt-1"><b>Pagamento:</b> {detalhe.pedido.pagamento}</p>{detalhe.pedido.observacao && <p className="mt-1"><b>Obs.:</b> {detalhe.pedido.observacao}</p>}</div><button onClick={() => void alternarPago()} className={`mt-4 w-full rounded-xl py-3 text-sm font-bold ${detalhe.pedido.pago ? "bg-green-600 text-white" : "border-2 border-green-600 text-green-700"}`}>{detalhe.pedido.pago ? "✓ PAGO ANTECIPADO" : "Marcar como PAGO ANTECIPADO"}</button>{!detalhe.pedido.pago && <p className="mt-2 text-center text-xs text-stone-500">Sem confirmação: pagamento na entrega ou balcão.</p>}<h3 className="mt-5 font-bold">Itens</h3>{detalhe.itens.map((item) => <div key={item.id} className="mt-2 flex justify-between rounded-xl border p-3 text-sm"><span>{item.quantidade}x {item.nome_produto}</span><b>{moeda.format(item.preco_unitario * item.quantidade)}</b></div>)}<div className="mt-4 flex justify-between border-t pt-4 text-lg font-bold"><span>Total</span><span>{moeda.format(detalhe.pedido.total)}</span></div><div className="mt-4 grid grid-cols-2 gap-2">{detalhe.pedido.status === "recebido" && <button onClick={() => setEdicao({ ...detalhe.pedido })} className="rounded-xl border py-3 text-sm font-bold">Editar</button>}<button onClick={() => void imprimir(detalhe.pedido, detalhe.itens)} className="rounded-xl bg-stone-900 py-3 text-sm font-bold text-white">Imprimir</button>{detalhe.pedido.status === "recebido" && <button onClick={() => void alterarStatus(detalhe.pedido.id, "preparando")} className="rounded-xl bg-orange-500 py-3 text-sm font-bold text-white">Iniciar produção</button>}<button onClick={() => void excluir()} className="rounded-xl py-3 text-sm font-bold text-red-600">Excluir</button></div><label className="mt-4 block text-sm font-bold">Status<select value={detalhe.pedido.status} onChange={(event) => void alterarStatus(detalhe.pedido.id, event.target.value)} className="campo mt-2"><option value="recebido">Novo</option><option value="preparando">Em produção</option>{detalhe.pedido.tipo_atendimento === "retirada" && <option value="pronto_para_retirada">Pronto para retirada</option>}<option value="saiu_para_entrega">Em rota de entrega</option><option value="finalizado">Finalizado e mover ao histórico</option><option value="cancelado">Cancelar e mover ao histórico</option></select></label></section></div>}

    {balcao && <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-3 sm:grid sm:place-items-center sm:p-4"><form onSubmit={salvarBalcao} className="mx-auto my-3 w-full max-w-3xl rounded-3xl bg-white p-5 shadow-2xl sm:my-0"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-orange-600">ATENDIMENTO RÁPIDO</p><h2 className="text-2xl font-bold">Novo pedido de balcão</h2><p className="mt-1 text-sm text-stone-500">Escolha os produtos, confira o total e registre o pedido.</p></div><button type="button" onClick={fecharBalcao} className="grid h-10 w-10 place-items-center rounded-full bg-stone-100 text-xl text-stone-600">×</button></div>{erro && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{erro}</p>}<div className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><section><label className="text-sm font-bold">Adicionar itens</label><input value={buscaBalcao} onChange={(event) => setBuscaBalcao(event.target.value)} placeholder="Buscar produto pelo nome" className="campo mt-2" /><div className="mt-3 grid max-h-80 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{produtosBalcao.length ? produtosBalcao.map((produto) => <button key={produto.id} type="button" onClick={() => adicionarItemBalcao(produto.id)} className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white p-3 text-left transition hover:border-orange-400 hover:bg-orange-50"><span className="min-w-0"><strong className="block truncate text-sm">{produto.nome}</strong><span className="text-xs text-orange-700">{moeda.format(produto.preco)}</span></span><span className="ml-3 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-stone-900 font-bold text-white">+</span></button>) : <p className="rounded-2xl border border-dashed p-4 text-sm text-stone-500 sm:col-span-2">Nenhum produto disponível para esta busca.</p>}</div></section><section className="rounded-2xl bg-stone-50 p-4"><div className="flex items-center justify-between"><h3 className="font-bold">Pedido atual</h3><span className="rounded-full bg-white px-2 py-1 text-xs font-bold text-stone-600">{itensBalcao.reduce((soma, item) => soma + item.quantidade, 0)} itens</span></div>{itensBalcao.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-stone-300 bg-white p-4 text-sm text-stone-500">Clique em um produto ao lado para montar o pedido.</p> : <div className="mt-3 space-y-2">{itensBalcao.map((item) => { const produto = produtos.find((atual) => atual.id === item.produtoId); if (!produto) return null; return <div key={item.produtoId} className="flex items-center gap-2 rounded-xl bg-white p-3 shadow-sm"><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{produto.nome}</p><p className="text-xs text-orange-700">{moeda.format(produto.preco * item.quantidade)}</p></div><button type="button" onClick={() => mudarQuantidadeBalcao(item.produtoId, -1)} className="grid h-8 w-8 place-items-center rounded-full bg-stone-100 font-bold">−</button><strong className="w-5 text-center">{item.quantidade}</strong><button type="button" onClick={() => mudarQuantidadeBalcao(item.produtoId, 1)} className="grid h-8 w-8 place-items-center rounded-full bg-stone-900 font-bold text-white">+</button></div>; })}</div>}<div className="mt-4 flex justify-between border-t border-stone-200 pt-4 text-lg font-bold"><span>Total</span><span className="text-orange-700">{moeda.format(totalBalcao)}</span></div></section></div><section className="mt-5 border-t pt-5"><h3 className="font-bold">Dados do atendimento</h3><div className="mt-3 grid gap-2 sm:grid-cols-2"><input name="cliente" required placeholder="Nome do cliente" className="campo" /><input name="telefone" placeholder="WhatsApp (opcional)" className="campo" /><select name="atendimento" className="campo"><option value="local">Consumir no restaurante</option><option value="retirada">Retirada</option><option value="entrega">Entrega</option></select><select name="pagamento" className="campo"><option>Dinheiro</option><option>Cartão</option><option>Pix</option></select><textarea name="endereco" placeholder="Endereço de entrega, se necessário" className="campo sm:col-span-2" /><textarea name="observacao" placeholder="Observação do pedido (opcional)" className="campo sm:col-span-2" /></div></section><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={fecharBalcao} className="rounded-xl border px-5 py-3 text-sm font-bold">Cancelar</button><button disabled={!itensBalcao.length} className="rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">Criar pedido · {moeda.format(totalBalcao)}</button></div></form></div>}
